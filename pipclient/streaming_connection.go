...
func makeStreamConns(addrs []string, streams int, timeout time.Duration, cb ConnectionStateNotificationCallback) ([]*streamConn, *connRetryPool) {
	total := len(addrs)
	if total > streams {
		total = streams
	}
	...
	for i := range conns {
		...
		conns[i] = newStreamConn(addrs[i], count, cb)
	}
...
func newStreamConn(addr string, streams int, cb ConnectionStateNotificationCallback) *streamConn {
	c := &streamConn{
		addr:    addr,
...
func (c *streamConn) connect() error {
	addr, err := c.enterConnect()
	if err != nil {
		return err
	}

	for {
		if err := c.tryConnect(addr, time.Second); err == nil || err == errStreamConnWrongState {
			break
		}
	}
...
func (c *streamConn) enterConnect() (string, error) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *streamConn) tryConnect(addr string, timeout time.Duration) (err error) {
	if c.notify != nil {
...
func (c *streamConn) newValidationStream() (pb.PIP_NewMappingStreamClient, error) {
	c.lock.RLock()
	state := c.state
	client := c.client
	c.lock.RUnlock()
	...
	return client.NewMappingStream(context.TODO())
}
